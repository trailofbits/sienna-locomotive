...
/*! The format for files (under the run directory) in which the program's initial minidump is
 * stored. */
...
/*! The format for files (under the run directory) in which the program's crash triage info is
 * stored. */
...
enum Event {
  /*! Request a new run ID from the server. WARNING: Deprecated; the server will complain and may
     die if you send this. */
  EVT_RUN_ID, // 0
  /*! Request a mutation from the server. WARNING: Deprecated; the server will complain and may die
     if you send this. */
  EVT_MUTATION, // 1
  /*! Request a replay from the server. */
  EVT_REPLAY, // 2
  /*! Request information about a run from the server. WARNING: Deprecated; the server will complain
     and may die if you send this. */
  EVT_RUN_INFO, // 3
  /*! Tell the server to finalize a run. WARNING: Deprecated; the server will complain and may die
     if you send this. */
  EVT_RUN_COMPLETE, // 4
  /*! Request a pathname for storing crash information from the server. WARNING: Deprecated; the
     server will complain and may die if you send this. */
  EVT_CRASH_PATH, // 5
  /*! Tell the server to end its session with this client. */
  EVT_SESSION_TEARDOWN, // 6
  /*! Request a pathname for storing a memory dump from the server. WARNING: Deprecated; the server
     will complain and may die if you send this. */
  EVT_MEM_DMP_PATH, // 7
  ...
  /*! Use this as a default value when handling multiple events. WARNING: The server will complain
     and may die if you send this. */
  EVT_INVALID = 255,
};
