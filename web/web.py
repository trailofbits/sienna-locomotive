...
def run():
    ...
    task = task_winafl_run.delay(run_id, run_cmd, config)
    print task.id
    cur.execute('INSERT INTO run_task_link_table (run_id, task_id) VALUES (?, ?)', [run_id, task.id])
    db.commit()
...
def run_kill():
    ...
    db = get_db()
    cur = db.cursor()
    cur.execute('UPDATE runs SET stopped = 1 WHERE rowid = ?', run_id)
    db.commit()
...
def task_winafl_run(run_id, run_cmd, config):
    with app.app_context():
        db = get_db()
        cur = db.cursor()
        cur.execute('UPDATE runs SET start_time = ?, stopped = 0 WHERE rowid = ?', [int(time.time()), run_id])
        db.commit()

    proc = winafl(run_cmd, config)
    ...
    while True:
        ...
        with app.app_context():    
            db = get_db()
            cur = db.cursor()
            stopped_query = 'SELECT stopped FROM runs WHERE rowid = ?'
            cur.execute(stopped_query, run_id)
            lresults = cur.fetchall()
            stopped = bool(lresults[0][0])

        if stopped or tick > 60*config['time_limit']:
            break
